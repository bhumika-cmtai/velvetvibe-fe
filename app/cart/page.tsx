...
export default function CartPage() {
  ...
  if (loading && items.length === 0) {
    return (
        <div className="min-h-screen">
            <Navbar />
            <div className="flex h-[60vh] items-center justify-center">Loading your cart...</div>
            <Footer />
        </div>
    );
  }
  ...
  if (error) {
    return (
        <div className="min-h-screen">
            <Navbar />
            <div className="flex h-[60vh] items-center justify-center text-red-500">Error: {error}</div>
            <Footer />
        </div>
    );
  }
      ...
      <main className="container mx-auto px-4 py-8">
        ...
        >
          <div className="flex items-center space-x-2 sm:space-x-4">
            ...
            <h1 className="text-xl sm:text-2xl font-serif font-bold">Shopping Cart ({items.reduce((acc, item) => acc + item.quantity, 0)} items)</h1>
          </div>
        </motion.div>
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                    ...
                    <div className="flex items-center space-x-3 sm:space-x-4 min-w-0 flex-1">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <span className="font-semibold text-sm sm:text-base" style={{ color: "#AA7E3D" }}>₹{item.product.price.toLocaleString()}</span>
                      </div>
                    </div>
