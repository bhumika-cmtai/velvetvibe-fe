  ...
  >
    <div className="col-span-6 flex items-center gap-4">
      <Link href={`/products/${item.slug}`}>
        <div className="relative w-24 h-24 rounded-lg overflow-hidden flex-shrink-0 bg-[var(--theme-background)]">
          <Image src={item.images[0] || "/placeholder.svg"} alt={item.name} fill className="object-cover hover:scale-105 transition-transform duration-300" />
        </div>
      </Link>
      <div>
        <Link href={`/products/${item.slug}`} className="font-semibold text-gray-800 hover:text-[var(--theme-accent)] transition-colors">{item.name}</Link>
        {/* <p className="text-sm text-gray-500">Color: {item.color} / Size: {item.size}</p> */}
        <button onClick={() => onRemove(item._id, item.name)} className="text-xs text-red-500 hover:underline mt-2 flex items-center gap-1">
    ...
    <div className="col-span-3 flex justify-center">
      <div className="flex items-center border rounded-full">
        <Button variant="ghost" size="icon" onClick={() => onUpdate(item._id, item.quantity - 1)} className="h-8 w-8 rounded-full"><Minus className="h-4 w-4" /></Button>
        <span className="px-4 font-medium text-sm w-12 text-center">{item.quantity}</span>
        <Button variant="ghost" size="icon" onClick={() => onUpdate(item._id, item.quantity + 1)} className="h-8 w-8 rounded-full"><Plus className="h-4 w-4" /></Button>
      </div>
    </div>
...
export default function CartPage() {
  ...
  return (
    <div className="min-h-screen bg-[var(--theme-background)]">
            ...
            <AnimatePresence>
              {cartItems.map((item, index) => (
                <div key={item._id} className={` ${index !== cartItems.length - 1 ? 'border-b' : ''}`}>
                  <CartItemCard item={item} onUpdate={handleUpdateQuantity} onRemove={handleRemoveFromCart} />
                </div>
              ))}
            </AnimatePresence>
          </div>
          ...
          <motion.div initial={{ opacity: 0, x: 50 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.3 }} className="lg:col-span-1">
            <div className="bg-[var(--theme-card-background)] rounded-2xl shadow-sm p-6 sticky top-28">
              ...
              <div className="space-y-3 mb-6 text-gray-700">
                <div className="flex justify-between items-center"><span className="text-gray-500">Subtotal</span><span className="font-semibold">₹{totalPrice.toLocaleString()}</span></div>
      ...
      {/* <RecommendedProducts cartItems={cartItems.map(item => item._id)} /> */}
      <Footer />
    </div>
  )
}