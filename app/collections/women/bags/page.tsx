...
// --- HELPERS ---
const getBagSubCategories = (products: Product[]) => {
  const subCategories = new Set<string>()
  products.forEach((product) => {
    const name = product.name?.toLowerCase() || ""
    if (name.includes("handbag")) subCategories.add("Handbag")
    if (name.includes("sling")) subCategories.add("Sling Bag")
    if (name.includes("backpack")) subCategories.add("Backpack")
    if (name.includes("tote")) subCategories.add("Tote Bag")
  })
...
export default function BagsCollectionPage() {
  ...
  // Fetch products (BAGS)
  useEffect(() => {
    dispatch(fetchProducts({ type: "bag", limit: 100 }) as any)
  }, [dispatch])

  // Get subcategories dynamically
  const subCategories = useMemo(
    () => getBagSubCategories(productsToFilter),
    [productsToFilter]
  )
      ...
      <main className="container mx-auto px-4 py-12">
        ...
        >
          ...
          <SectionTitle
            ...
            subtitle="Explore our stylish range of bags, crafted for both elegance and convenience."
            isSparkling={true}
            className="mb-12"
          />
        </motion.div>
