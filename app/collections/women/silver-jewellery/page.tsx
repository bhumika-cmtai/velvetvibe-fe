...
// Helper to get unique categories from product names
const getSubCategories = (products: Product[]) => {
  ...
  products.forEach((product) => {
    ...
    // Add more rules here as your product catalog grows
  })
  return ["All", ...Array.from(subCategories)]
}

export default function SilverJewelleryPage() {
  const silverProducts = useMemo(() => womenProducts.filter((p) => p.category === "silver"), [])

  const [activeSubCategory, setActiveSubCategory] = useState("All")
  const [priceRange, setPriceRange] = useState([0, 5000])
  
  const subCategories = useMemo(() => getSubCategories(silverProducts), [silverProducts])
  const maxPrice = useMemo(() => {
    return Math.ceil(Math.max(...silverProducts.map(p => p.priceDiscounted), 0) / 100) * 100
  }, [silverProducts])
  ...
  }, [silverProducts, activeSubCategory, priceRange])
      ...
      <main className="container mx-auto px-4 py-12">
        ...
        <div className="flex flex-wrap items-center gap-4 mb-12">
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent 
              className="w-64 p-4" 
              onCloseAutoFocus={(e) => e.preventDefault()} // Prevents closing when interacting with slider
            >
